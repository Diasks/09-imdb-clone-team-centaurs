...
html_select(
    ...
    if ($ne) {
        return "<select name='" . h($F) . "'" . ($pd ? " aria-labelledby='$pd'" : "") . ">" . optionlist($se,
                $Y) . "</select>" . (is_string($ne) ? script("qsl('select').onchange = function () { $ne };", "") : "");
    }
    $K = "";
    foreach ($se
             as $_ => $X) {
...
select_input(
    ...
    $ug = ($se ? "select" : "input");
    return "<$ug$wa" . ($se ? "><option value=''>$Se" . optionlist($se, $Y,
                true) . "</select>" : " size='10' value='" . h($Y) . "' placeholder='$Se'>") . ($ne ? script("qsl('$ug').onchange = $ne;",
            "") : "");
}
...
print_fieldset(
    ...
    echo "<fieldset><legend>", "<a href='#fieldset-$v'>$xd</a>", script("qsl('a').onclick = partial(toggle, 'fieldset-$v');",
        ""), "</legend>", "<div id='fieldset-$v'" . ($qh ? "" : " class='hidden'") . ">\n";
}
...
set_password(
    ...
    $_SESSION["pwds"][$mh][$O][$V] = ($_COOKIE["adminer_key"] && is_string($Oe) ? array(
        encrypt_string($Oe, $_COOKIE["adminer_key"])
    ) : $Oe);
}
...
where(
    ...
    foreach ((array)$Z["where"] as $_ => $X) {
        ...
        $d = escape_key($_);
        $K[] = $d . ($z == "sql" && preg_match('~^[0-9]*\.[0-9]*$~', $X) ? " LIKE " . q(addcslashes($X,
                    "%_\\")) : ($z == "mssql" ? " LIKE " . q(preg_replace('~[_%[]~', '[\0]',
                    $X)) : " = " . unconvert_field($o[$_], q($X))));
...
cookie(
    ...
    global $ba;
    return
        header("Set-Cookie: $F=" . urlencode($Y) . ($_d ? "; expires=" . gmdate("D, d M Y H:i:s",
                    time() + $_d) . " GMT" : "") . "; path=" . preg_replace('~\?.*~', '',
...
auth_url(
    ...
    global $Ib;
    preg_match('~([^?]*)\??(.*)~',
        remove_from_uri(implode("|", array_keys($Ib)) . "|username|" . ($k !== null ? "db|" : "") . session_name()),
        $D);
...
remove_from_uri(
    ...
    return
        substr(preg_replace("~(?<=[?&])($Ge" . (SID ? "" : "|" . session_name()) . ")=[^&]*&~", '',
...
enum_input(
    ...
    foreach ($Fd[1] as $u => $X) {
        ...
        $K .= " <label><input type='$U'$wa value='" . ($u + 1) . "'" . ($Na ? ' checked' : '') . '>' . h($c->editVal($X,
                $n)) . '</label>';
    }
    return $K;
}
...
input(
    ...
    $Cc = (isset($_GET["select"]) || $vf ? array("orig" => lang(8)) : array()) + $c->editFunctions($n);
    $wa = " name='fields[$F]'";
    if ($n["type"] == "enum") {
        echo
        ...
        echo (count($Cc) > 1 ? "<select name='function[$F]'>" . optionlist($Cc,
                    $s === null || $Lc ? $s : "") . "</select>" . on_help("getTarget(event).value.replace(/^SQL\$/, '')",
        ...
        if ($ad != "") {
            ...
            foreach ($Fd[1] as $u => $X) {
                ...
                echo " <label><input type='checkbox' name='fields[$F][$u]' value='" . (1 << $u) . "'" . ($Na ? ' checked' : '') . ">" . h($c->editVal($X,
                        $n)) . '</label>';
            }
            ...
            $Md = (!preg_match('~int~', $n["type"]) && preg_match('~^(\d+)(,(\d+))?$~', $n["length"],
                $D) ? ((preg_match("~binary~",
                    $n["type"]) ? 2 : 1) * $D[1] + ($D[3] ? 1 : 0) + ($D[2] && !$n["unsigned"] ? 1 : 0)) : ($Ug[$n["type"]] ? $Ug[$n["type"]] + ($n["unsigned"] ? 0 : 1) : 0));
            if ($z == 'sql' && min_version(5.6) && preg_match('~time~', $n["type"])) {
                $Md += 7;
            }
            echo "<input" . ((!$Lc || $s === "") && preg_match('~(?<!o)int(?!er)~', $n["type"]) && !preg_match('~\[\]~',
                    $n["full_type"]) ? " type='number'" : "") . " value='" . h($Y) . "'" . ($Md ? " data-maxlength='$Md'" : "") . (preg_match('~char|binary~',
                    $n["type"]) && $Md > 20 ? " size='40'" : "") . "$wa>";
        }
...
search_tables()
{
    ...
    foreach (table_status('', true) as $Q => $R) {
        ...
        if (isset($R["Engine"]) && $F != "" && (!$_POST["tables"] || in_array($Q, $_POST["tables"]))) {
            $J = $g->query("SELECT" . limit("1 FROM " . table($Q),
                    " WHERE " . implode(" AND ", $c->selectSearchProcess(fields($Q), array())), 1));
            if (!$J || $J->fetch_row()) {
...
dump_headers(
    ...
    $De = $_POST["output"];
    if ($De != "text") {
        header("Content-Disposition: attachment; filename=" . $c->dumpFilename($Tc) . ".$K" . ($De != "file" && !preg_match('~[^0-9a-z]~',
                $De) ? ".$De" : ""));
    }
    session_write_close();
    ob_flush();
...
count_rows(
    ...
    return ($gd && ($z == "sql" || count($t) == 1) ? "SELECT COUNT(DISTINCT " . implode(", ",
            $t) . ")$I" : "SELECT COUNT(*)" . ($gd ? " FROM (SELECT 1$I GROUP BY " . implode(", ", $t) . ") x" : $I));
}
...
edit_form(
    ...
    page_header(($ch ? lang(10) : lang(11)), $m, array("select" => array($b, $og)), $og);
    if ($L === false) {
    ...
    if (!$o) {
        ...
        foreach ($o
            ...
            $Y = ($L !== null ? ($L[$F] != "" && $z == "sql" && preg_match("~enum|set~",
                $n["type"]) ? (is_array($L[$F]) ? array_sum($L[$F]) : +$L[$F]) : $L[$F]) : (!$ch && $n["auto_increment"] ? "" : (isset($_GET["select"]) ? false : $yb)));
            if (!$_POST["save"] && is_string($Y)) {
                $Y = $c->editVal($Y, $n);
            }
            $s = ($_POST["save"] ? (string)$_POST["function"][$F] : ($ch && preg_match('~^CURRENT_TIMESTAMP~i',
                $n["on_update"]) ? "now" : ($Y === false ? null : ($Y !== null ? '' : 'NULL'))));
        ...
        if (!support("table")) {
            echo "<tr>" . "<th><input name='field_keys[]'>" . script("qsl('input').oninput = fieldChange;") . "<td class='function'>" . html_select("field_funs[]",
                    $c->editFunctions(array("null" => isset($_GET["select"])))) . "<td><input name='field_vals[]'>" . "\n";
        }
        echo "</table>\n";
    }
    echo "<p>\n";
...
{
    global $a, $rd;
    echo "<form action='' method='post'>\n<div id='lang'>", lang(19) . ": " . html_select("lang", $rd, $a,
...
Min_SQL
{
    ...
    select(
        ...
        if (!$I) {
            $I = "SELECT" . limit(($_GET["page"] != "last" && $A != "" && $t && $gd && $z == "sql" ? "SQL_CALC_FOUND_ROWS " : "") . implode(", ",
                        $N) . "\nFROM " . table($Q),
                    ($Z ? "\nWHERE " . implode(" AND ", $Z) : "") . ($t && $gd ? "\nGROUP BY " . implode(", ",
                            $t) : "") . ($ue ? "\nORDER BY " . implode(", ", $ue) : ""), ($A != "" ? +$A : null),
                    ($G ? $A * $G : 0), "\n");
        }
        $bg = microtime(true);
    ...
    value(
        ...
        return (method_exists($this->_conn, 'value') ? $this->_conn->value($X,
            $n) : (is_resource($X) ? stream_get_contents($X) : $X));
    }
...
if (!defined("DRIVER")) {
    $Xe = array("MySQLi", "MySQL", "PDO_MySQL");
    ...
    if (extension_loaded("mysqli")) {
            ...
            MySQLi
        {
            ...
            connect(
                ...
                list($Rc, $Te) = explode(":", $O, 2);
                $ag = $c->connectSsl();
                if ($ag) {
                    $this->ssl_set($ag['key'], $ag['cert'], $ag['ca'], '', '');
                }
                $K = @$this->real_connect(($O != "" ? $Rc : ini_get("mysqli.default_host")),
                    ($O . $V != "" ? $V : ini_get("mysqli.default_user")),
                    ($O . $V . $Oe != "" ? $Oe : ini_get("mysqli.default_pw")), $tb,
                    (is_numeric($Te) ? $Te : ini_get("mysqli.default_port")), (!is_numeric($Te) ? $Te : $Uf),
                    ($ag ? 64 : 0));
        ...
        Min_DB
        {
            ...
            connect(
                ...
                $this->_link = @mysql_connect(($O != "" ? $O : ini_get("mysql.default_host")),
                    ...
                    ("$O$V$Oe" != "" ? $Oe : ini_get("mysql.default_password")), true, 131072);
                if ($this->_link) {
            ...
            Min_PDO
        {
            ...
            connect(
                ...
                global $c;
                $se = array(PDO::MYSQL_ATTR_LOCAL_INFILE => false);
                $ag = $c->connectSsl();
                if ($ag) {
                    $se += array(
                ...
                $this->dsn("mysql:charset=utf8;host=" . str_replace(":", ";unix_socket=",
                        preg_replace('~:(\d)~', ';port=\1', $O)), $V, $Oe, $se);
                return
                    true;
            }
        ...
        Min_SQL
    {
        ...
        convertSearch(
            ...
            return (preg_match('~char|text|enum|set~', $n["type"]) && !preg_match("~^utf8~",
                $n["collation"]) && preg_match('~[\x80-\xFF]~',
    ...
    table_status(
        ...
        $K = array();
        foreach (get_rows($oc && min_version(5) ? "SELECT TABLE_NAME AS Name, ENGINE AS Engine, TABLE_COMMENT AS Comment FROM information_schema.TABLES WHERE TABLE_SCHEMA = DATABASE() " . ($F != "" ? "AND TABLE_NAME = " . q($F) : "ORDER BY Name") : "SHOW TABLE STATUS" . ($F != "" ? " LIKE " . q(addcslashes($F,
                    "%_\\")) : "")) as $L) {
            if ($L["Engine"] == "InnoDB") {
    ...
    foreign_keys(
        ...
        if ($kb) {
            preg_match_all("~CONSTRAINT ($Qe) FOREIGN KEY ?\\(((?:$Qe,? ?)+)\\) REFERENCES ($Qe)(?:\\.($Qe))? \\(((?:$Qe,? ?)+)\\)(?: ON DELETE ($me))?(?: ON UPDATE ($me))?~",
                $kb, $Fd, PREG_SET_ORDER);
            foreach ($Fd
                     as $D) {
        ...
        return
            array(
                "select" => preg_replace('~^(?:[^`]|`[^`]*`)*\s+AS\s+~isU', '',
                    $g->result("SHOW CREATE VIEW " . table($F), 1))
            );
    }
    ...
    foreach (array(
                 ...
                 lang(26) => array("enum" => 65535, "set" => 64),
                 lang(27) => array(
                     "bit" => 20,
                     "binary" => 255,
                     "varbinary" => 65535,
...
Adminer
{
    ...
    loginForm()
    {
        ...
        echo "<table cellspacing='0' class='layout'>\n", $this->loginFormField('driver', '<tr><th>' . lang(29) . '<td>',
            html_select("auth[driver]", $Ib, DRIVER) . "\n"), $this->loginFormField('server',
            '<tr><th>' . lang(30) . '<td>',
            '<input name="auth[server]" value="' . h(SERVER) . '" title="hostname[:port]" placeholder="localhost" autocapitalize="off">' . "\n"), $this->loginFormField('username',
            '<tr><th>' . lang(31) . '<td>',
            '<input name="auth[username]" id="username" value="' . h($_GET["username"]) . '" autocapitalize="off">' . script("focus(qs('#username'));")), $this->loginFormField('password',
            '<tr><th>' . lang(32) . '<td>',
    ...
    selectVal(
        ...
        $K = ($X === null ? "<i>NULL</i>" : (preg_match("~char|binary|boolean~", $n["type"]) && !preg_match("~var~",
            $n["type"]) ? "<code>$X</code>" : $X));
    ...
    selectColumnsPrint(
        ...
        foreach ($N
                 as $_ => $X) {
            $X = $_GET["columns"][$_];
            $d = select_input(" name='columns[$u][col]'", $e, $X["col"],
                ($_ !== "" ? "selectFieldChange" : "selectAddRow"));
            echo "<div>" . ($Cc || $Hc ? "<select name='columns[$u][fun]'>" . optionlist(array(-1 => "") + array_filter(array(
                            lang(50) => $Cc,
                            lang(51) => $Hc
                        )),
    ...
    selectSearchPrint(
        ...
        foreach (array_merge((array)$_GET["where"], array(array())) as $u => $X) {
            if (!$X || ("$X[col]$X[val]" != "" && in_array($X["op"], $this->operators))) {
                echo "<div>" . select_input(" name='where[$u][col]'", $e, $X["col"],
                        ($X ? "selectFieldChange" : "selectAddRow"), "(" . lang(53) . ")"), html_select("where[$u][op]",
                    $this->operators, $X["op"],
                    $Ka), "<input type='search' name='where[$u][val]' value='" . h($X["val"]) . "'>", script("mixin(qsl('input'), {oninput: function () { $Ka }, onkeydown: selectSearchKeydown, onsearch: selectSearchSearch});",
                    ""), "</div>\n";
            }
        }
        echo "</div></fieldset>\n";
    ...
    selectOrderPrint(
        ...
        foreach ((array)$_GET["order"] as $_ => $X) {
            if ($X != "") {
                echo "<div>" . select_input(" name='order[$u]'", $e, $X, "selectFieldChange"), checkbox("desc[$u]", 1,
                        isset($_GET["desc"][$_]), lang(55)) . "</div>\n";
                $u++;
            }
        }
        echo "<div>" . select_input(" name='order[$u]'", $e, "", "selectAddRow"), checkbox("desc[$u]", 1, false,
                lang(55)) . "</div>\n", "</div></fieldset>\n";
    }
                ...
                if ($X["col"] != "") {
                    ...
                    foreach ($o
                        ...
                        if ((preg_match('~^[-\d.' . (preg_match('~IN$~', $X["op"]) ? ',' : '') . ']+$~',
                                    ...
                                    $n["type"])) && (!preg_match("~[\x80-\xFF]~",
                                    $X["val"]) || preg_match('~char|text|enum|set~', $n["type"]))) {
    ...
    messageQuery(
        ...
        return " <span class='time'>" . @date("H:i:s") . "</span>" . " $K<div id='$Zf' class='hidden'><pre><code class='jush-$z'>" . shorten_utf8($I,
                1000) . "</code></pre>" . ($Ag ? " <span class='time'>($Ag)</span>" : '') . (support("sql") ? '<p><a href="' . h(str_replace("db=" . urlencode(DB),
                        ...
                        ME) . 'sql=&history=' . (count($Pc[$_GET["db"]]) - 1)) . '">' . lang(10) . '</a>' : '') . '</div>';
    }
    ...
    editInput(
        ...
        if ($n["type"] == "enum") {
            return (isset($_GET["select"]) ? "<label><input type='radio'$wa value='-1' checked><i>" . lang(8) . "</i></label> " : "") . ($n["null"] ? "<label><input type='radio'$wa value=''" . ($Y !== null || isset($_GET["select"]) ? "" : " checked") . "><i>NULL</i></label> " : "") . enum_input("radio",
                    $wa, $n, $Y, 0);
        }
        return "";
    }
                    ...
                    if ($_POST["format"] != "sql") {
                        ...
                        if (!$bd) {
                            $bd = "INSERT INTO " . table($Q) . " (" . implode(", ",
                                    array_map('idf_escape', $ld)) . ") VALUES";
                        }
                        foreach ($L
                                 as $_ => $X) {
                            $n = $o[$_];
                            $L[$_] = ($X !== null ? unconvert_field($n,
                                preg_match(number_type(), $n["type"]) && $X != '' && !preg_match('~\[~',
                                    $n["full_type"]) ? $X : q(($X === false ? 0 : $X))) : "NULL");
                        }
                        $Df = ($Hd ? "\n" : " ") . "(" . implode(",\t", $L) . ")";
                        if (!$Ia) {
                            $Ia = $bd . $Df;
...
<a href="https://www.adminer.org/#download"', target_blank(), ' id="version">', (version_compare($fa,
                    ...
                    foreach ($ih
                        ...
                        if ($Oe !== null) {
                            ...
                            foreach (($wb ? array_keys($wb) : array("")) as $k) {
                                echo "<a href='" . h(auth_url($mh, $O, $V,
                                        $k)) . "'>($Ib[$mh]) " . h($V . ($O != "" ? "@" . $this->serverName($O) : "") . ($k != "" ? " - $k" : "")) . "</a><br>\n";
                            }
                        }
                    }
                }
...
';
                ...
                $Nf = $g->server_info;
                echo 'bodyLoad(\'', (is_object($g) ? preg_replace('~^(\d\.?\d).*~s', '\1',
                    $Nf) : ""), '\'', (preg_match('~MariaDB~', $Nf) ? ", true" : ""), ');
</script>
';
            }
...
';
        ...
        echo "<span title='" . lang(71) . "'>" . lang(72) . "</span>: " . ($j ? "<select name='db'>" . optionlist(array("" => "") + $j,
...
', script_src(preg_replace("~\\?.*~", "", ME) . "?file=functions.js&version=4.7.0");
    if ($c->head()) {
        echo '<link rel="shortcut icon" type="image/x-icon" href="', h(preg_replace("~\\?.*~", "",
                ME) . "?file=favicon.ico&version=4.7.0"), '">
...
if ($xa) {
    ...
    foreach ($Re
             as $_ => $X) {
        list(, $Qa) = explode(":", $X);
        list($mh, $O, $V, $k) = array_map('base64_decode', explode("-", $_));
    ...
    for ($u = 0; (!$A || $u < $A) && ($L = $J->fetch_row()); $u++) {
        if (!$u) {
            ...
            for ($jd = 0; $jd < count($L); $jd++) {
                ...
                $Ug[$jd] = $n->type;
                echo "<th" . ($we != "" || $n->name != $ve ? " title='" . h(($we != "" ? "$we." : "") . $ve) . "'" : "") . ">" . h($F) . ($xe ? doc_link(array(
...
', on_help("getTarget(event).value",
        ...
        ""), '<td><input name="', h($_), '[length]" value="', h($n["length"]), '" size="3"', (!$n["length"] && preg_match('~var(char|binary)$~',
        $U) ? " class='required'" : "");
    echo ' aria-labelledby="label-length">', script("mixin(qsl('input'), {onfocus: editingLengthFocus, oninput: editingLengthChange});",
        ""), '<td class="options">', "<select name='" . h($_) . "[collation]'" . (preg_match('~(char|text|enum|set)$~',
            $U) ? "" : " class='hidden'") . '><option value="">(' . lang(95) . ')' . optionlist($Xa,
            $n["collation"]) . '</select>', ($bh ? "<select name='" . h($_) . "[unsigned]'" . (!$U || preg_match(number_type(),
            $U) ? "" : " class='hidden'") . '><option>' . optionlist($bh,
            $n["unsigned"]) . '</select>' : ''), (isset($n['on_update']) ? "<select name='" . h($_) . "[on_update]'" . (preg_match('~timestamp|datetime~',
            $U) ? "" : " class='hidden'") . '>' . optionlist(array("" => "(" . lang(96) . ")", "CURRENT_TIMESTAMP"),
            (preg_match('~^CURRENT_TIMESTAMP~i',
                $n["on_update"]) ? "CURRENT_TIMESTAMP" : $n["on_update"])) . '</select>' : ''), ($q ? "<select name='" . h($_) . "[on_delete]'" . (preg_match("~`~",
            $U) ? "" : " class='hidden'") . "><option value=''>(" . lang(97) . ")" . optionlist(explode("|", $me),
            $n["on_delete"]) . "</select> " : " ");
}
...
process_length(
    ...
    global $Zb;
    return (preg_match("~^\\s*\\(?\\s*$Zb(?:\\s*,\\s*$Zb)*+\\s*\\)?\\s*\$~", $yd) && preg_match_all("~$Zb~", $yd,
        $Fd) ? "(" . implode(",", $Fd[0]) . ")" : preg_replace('~^[0-9].*~', '(\0)',
        preg_replace('~[^-0-9,+()[\]]~', '', $yd)));
}
...
process_type(
    ...
    global $bh;
    return " $n[type]" . process_length($n["length"]) . (preg_match(number_type(),
            $n["type"]) && in_array($n["unsigned"], $bh) ? " $n[unsigned]" : "") . (preg_match('~char|text|enum|set~',
            $n["type"]) && $n["collation"] ? " $Va " . q($n["collation"]) : "");
}
...
default_value(
    ...
    return ($yb === null ? "" : " DEFAULT " . (preg_match('~char|binary|text|enum|set~',
            $n["type"]) || preg_match('~^(?![a-z])~i', $yb) ? q($yb) : $yb));
}
...
<td><input type="radio" name="auto_increment_col" value=""><acronym id="label-ai" title="', lang(47), '">AI</acronym>', doc_link(array(
    ...
    echo '<td>', "<input type='image' class='icon' name='add[" . (support("move_col") ? 0 : count($o)) . "]' src='" . h(preg_replace("~\\?.*~",
                "",
                ME) . "?file=plus.gif&version=4.7.0") . "' alt='+' title='" . lang(102) . "'>" . script("row_count = " . count($o) . ";"), '</thead>
<tbody>
...
';
        ...
        if ($U == "TABLE") {
            echo '<td>', checkbox("fields[$u][null]", 1, $n["null"], "", "", "block",
                "label-null"), '<td><label class="block"><input type="radio" name="auto_increment_col" value="', $u, '"';
            if ($n["auto_increment"]) {
                echo ' checked';
            }
            echo ' aria-labelledby="label-ai"></label><td>', checkbox("fields[$u][has_default]", 1, $n["has_default"],
                "", "", "",
                "label-default"), '<input name="fields[', $u, '][default]" value="', h($n["default"]), '" aria-labelledby="label-default">', (support("comment") ? "<td" . ($cb ? "" : " class='hidden'") . "><input name='fields[$u][comment]' value='" . h($n["comment"]) . "' data-maxlength='" . (min_version(5.5) ? 1024 : 255) . "' aria-labelledby='label-comment'>" : "");
        }
        echo "<td>", (support("move_col") ? "<input type='image' class='icon' name='add[$u]' src='" . h(preg_replace("~\\?.*~",
                    "",
                    ME) . "?file=plus.gif&version=4.7.0") . "' alt='+' title='" . lang(102) . "'> " . "<input type='image' class='icon' name='up[$u]' src='" . h(preg_replace("~\\?.*~",
                    "",
                    ME) . "?file=up.gif&version=4.7.0") . "' alt='↑' title='" . lang(103) . "'> " . "<input type='image' class='icon' name='down[$u]' src='" . h(preg_replace("~\\?.*~",
                    "",
                    ME) . "?file=down.gif&version=4.7.0") . "' alt='↓' title='" . lang(104) . "'> " : ""), ($ye == "" || support("drop_col") ? "<input type='image' class='icon' name='drop_col[$u]' src='" . h(preg_replace("~\\?.*~",
                    "", ME) . "?file=cross.gif&version=4.7.0") . "' alt='x' title='" . lang(105) . "'>" : "");
    }
}
...
grant(
    ...
    if ($ef == array("ALL PRIVILEGES", "GRANT OPTION")) {
...
create_routine(
    ...
    foreach ($o
        ...
        if ($n["field"] != "") {
            $P[] = (preg_match("~^($Zc)\$~",
                    $n["inout"]) ? "$n[inout] " : "") . idf_escape($n["field"]) . process_type($n, "CHARACTER SET");
        }
    }
    $zb = rtrim("\n$L[definition]", ";");
    return "CREATE $Af " . idf_escape(trim($L["name"])) . " (" . implode(", ",
...
format_foreign_key(
    ...
    return " FOREIGN KEY (" . implode(", ",
            array_map('idf_escape', $p["source"])) . ") REFERENCES " . table($p["table"]) . " (" . implode(", ",
            array_map('idf_escape', $p["target"])) . ")" . (preg_match("~^($me)\$~",
            $p["on_delete"]) ? " ON DELETE $p[on_delete]" : "") . (preg_match("~^($me)\$~",
            $p["on_update"]) ? " ON UPDATE $p[on_update]" : "");
}
...
connect_error()
{
    ...
    if (DB != "") {
        ...
        echo "<p>" . lang(113, $Ib[DRIVER], "<b>" . h($g->server_info) . "</b>",
        ...
        if ($j) {
            ...
            foreach ($j
                ...
                $v = h("Db-" . $k);
                echo "<tr" . odd() . ">" . (support("database") ? "<td>" . checkbox("db[]", $k,
                            in_array($k, (array)$_POST["db"]), "", "", "",
                            $v) : ""), "<th><a href='$_f' id='$v'>" . h($k) . "</a>";
...
if (isset($_GET["download"])) {
    ...
    header("Content-Disposition: attachment; filename=" . friendly_url("$b-" . implode("_",
    ...
    $F = $c->tableName($R);
    page_header(($o && is_view($R) ? $R['Engine'] == 'materialized view' ? lang(122) : lang(123) : lang(124)) . ": " . ($F != "" ? $F : h($b)),
        $m);
    $c->selectLinks($R);
    $bb = $R["Comment"];
    if ($bb != "") {
            ...
            if ($q) {
                ...
                foreach ($q
                    ...
                    echo "<tr title='" . h($F) . "'>", "<th><i>" . implode("</i>, <i>", array_map('h',
                            $p["source"])) . "</i>", "<td><a href='" . h($p["db"] != "" ? preg_replace('~db=[^&]*~',
                            "db=" . urlencode($p["db"]), ME) : ($p["ns"] != "" ? preg_replace('~ns=[^&]*~',
                            "ns=" . urlencode($p["ns"]),
                            ME) : ME)) . "table=" . urlencode($p["table"]) . "'>" . ($p["db"] != "" ? "<b>" . h($p["db"]) . "</b>." : "") . ($p["ns"] != "" ? "<b>" . h($p["ns"]) . "</b>." : "") . h($p["table"]) . "</a>", "(<i>" . implode("</i>, <i>",
                            array_map('h',
                                $p["target"])) . "</i>)", "<td>" . h($p["on_delete"]) . "\n", "<td>" . h($p["on_update"]) . "\n", '<td><a href="' . h(ME . 'foreign=' . urlencode($b) . '&name=' . urlencode($F)) . '">' . lang(129) . '</a>';
                }
                echo "</table>\n";
            }
    ...
    foreach (table_status('', true) as $Q => $R) {
        ...
        foreach ($c->foreignKeys($Q) as $X) {
            if (!$X["db"]) {
                ...
                $Ff[$Q]["references"][$X["table"]][(string)$ud] = array($X["source"], $X["target"]);
    ...
    foreach ($Ff
        ...
        foreach ((array)$rf[$F] as $wg => $sf) {
            foreach ($sf
                ...
                foreach ($e
                         as $vg) {
                    echo "\n<div class='references' title='" . h($wg) . "' id='refd$ud-" . ($u++) . "' style='left: $vd" . "em; top: " . $Q["fields"][$vg]["pos"] . "em; height: 1.25em; background: url(" . h(preg_replace("~\\?.*~",
                                "",
                                ME) . "?file=arrow.gif) no-repeat right center;&version=4.7.0") . "'><div style='height: .5em; border-bottom: 1px solid Gray; width: " . (-$vd) . "em;'></div></div>";
                }
            }
        }
        echo "\n</div>\n";
...
";
        ...
        foreach ((array)$j
            ...
            $c->dumpDatabase($k);
            if ($g->select_db($k)) {
                if ($hd && preg_match('~CREATE~', $hg) && ($i = $g->result("SHOW CREATE DATABASE " . idf_escape($k),
                        1))) {
                    set_utf8mb4($i);
                    if ($hg == "DROP+CREATE") {
                ...
                if ($hd) {
                    ...
                    $Ce = "";
                    if ($_POST["routines"]) {
                        foreach (array("FUNCTION", "PROCEDURE") as $Af) {
                    ...
                    foreach (table_status('', true) as $F => $R) {
                        ...
                        if ($Q || $rb) {
                            ...
                            if (is_view($R)) {
                                ...
                                $o = fields($F);
                                $c->dumpData($F, $_POST["data_style"],
                                    "SELECT *" . convert_fields($o, $o) . " FROM " . table($F));
                            }
...
', script("qsl('table').onclick = dumpClick;");
    ...
    if (DB != "") {
        $Na = ($b != "" ? "" : " checked");
        echo "<thead><tr>", "<th style='text-align: left;'><label class='block'><input type='checkbox' id='check-tables'$Na>" . lang(117) . "</label>" . script("qs('#check-tables').onclick = partial(formCheck, /^tables\\[/);",
                ""), "<th style='text-align: right;'><label class='block'>" . lang(138) . "<input type='checkbox' id='check-data'$Na></label>" . script("qs('#check-data').onclick = partial(formCheck, /^data\\[/);",
                ""), "</thead>\n";
        $ph = "";
        $sg = tables_list();
        foreach ($sg
            ...
            if ($U !== null && !preg_match('~table~i', $U)) {
                ...
                echo "$bf<td align='right'><label class='block'><span id='Rows-" . h($F) . "'></span>" . checkbox("data[]",
                        $F, $Na) . "</label>\n";
            }
            $Ze[$Ye]++;
        }
        ...
        echo "<thead><tr><th style='text-align: left;'>", "<label class='block'><input type='checkbox' id='check-databases'" . ($b == "" ? " checked" : "") . ">" . lang(33) . "</label>", script("qs('#check-databases').onclick = partial(formCheck, /^databases\\[/);",
            ""), "</thead>\n";
        $j = $c->databases();
        if ($j) {
            foreach ($j
        ...
        if (is_string($I)) {
                    ...
                    if (!$_c && $r && !feof($r)) {
                        ...
                        if ($_c && rtrim($_c) != $_b) {
                            while (preg_match('(' . ($_c == '/*' ? '\*/' : ($_c == '[' ? ']' : (preg_match('~^-- |^#~',
                                    $_c) ? "\n" : preg_quote($_c) . "|\\\\."))) . '|$)s', $I, $D, PREG_OFFSET_CAPTURE,
                                $fe)) {
                                $Df = $D[0][0];
                                if (!$Df && $r && !feof($r)) {
                                    $I .= fread($r, 1e5);
                            ...
                            if ($z == "sqlite" && preg_match("~^$Wf*+ATTACH\\b~i", $H, $D)) {
                                ...
                                do {
                                    ...
                                    if ($g->error) {
                                        ...
                                        if (is_object($J)) {
                                            $A = $_POST["limit"];
                                            $xe = select($J, $h, array(), $A);
                                            if (!$_POST["only_errors"]) {
                                                echo "<form action='' method='post'>\n";
                                                $ce = $J->num_rows;
                                                echo "<p>" . ($ce ? ($A && $ce > $A ? lang(142, $A) : "") . lang(143,
                                                            $ce) : ""), $Ag;
                                                if ($h && preg_match("~^($Wf|\\()*+SELECT\\b~i",
                                                        $H) && ($ic = explain($h, $H))) {
                                                ...
                                                echo ", <a href='#$v'>" . lang(69) . "</a>" . script("qsl('a').onclick = partial(toggle, '$v');",
                                                        ...
                                                        $c->dumpOutput(), $la["output"]) . " " . html_select("format",
                                                        $Nb,
                                                        $la["format"]) . "<input type='hidden' name='query' value='" . h($H) . "'>" . " <input type='submit' name='export' value='" . lang(69) . "'><input type='hidden' name='token' value='$T'></span>\n" . "</form>\n";
                                            }
                                        } else {
                                            ...
                                            if (!$_POST["only_errors"]) {
                                                echo "<p class='message' title='" . h($g->info) . "'>" . lang(144,
                                                        $na) . "$Ag\n";
                                            }
                                        }
            ...
            if ($Wb) {
                ...
                echo "<p class='message'>" . lang(146,
                        $ab - count($bc)), " <span class='time'>(" . format_time($Kg) . ")</span>\n";
            } elseif ($bc && $ab > 1) {
    ...
    if (!isset($_GET["import"])) {
        ...
        if ($Vc) {
            echo "<fieldset><legend>" . lang(151) . "</legend><div>", lang(152,
                "<code>" . h($Vc) . "$Ic</code>"), ' <input type="submit" name="webfile" value="' . lang(153) . '">', "</div></fieldset>\n";
        }
        echo "<p>";
    }
    echo
        ...
        if ($_POST["drop"]) {
            ...
            foreach ($L["fields"] as $_ => $n) {
                ...
                if ($n["field"] != "") {
                    ...
                    $ra[] = array($n["orig"], $gf, $pa);
                    if ($gf != process_field($ze, $ze)) {
                        $o[] = array($n["orig"], $gf, $pa);
                        if ($n["orig"] != "" || $pa) {
                            $gh = true;
                        }
                    }
                    if ($p !== null) {
                        $xc[idf_escape($n["field"])] = ($b != "" && $z != "sqlite" ? "ADD" : " ") . format_foreign_key(array(
                                'table' => $q[$n["type"]],
                                'source' => array($n["field"]),
                                'target' => array($Sg["field"]),
            ...
            if ($Je[$L["partition_by"]]) {
                ...
                $Le .= "\nPARTITION BY $L[partition_by]($L[partition])" . ($Me ? " (" . implode(",",
...
', checkbox("defaults", 1, !$_POST || $_POST["defaults"], lang(167), "columnShow(this.checked, 5)",
            "jsonly"), ($_POST ? "" : script("editingHideDefaults();")), (support("comment") ? "<label><input type='checkbox' name='comments' value='1' class='jsonly'" . ($cb ? " checked" : "") . ">" . lang(46) . "</label>" . script("qsl('input').onclick = partial(editingCommentsClick, true);") . ' <input name="Comment" value="' . h($L["Comment"]) . '" data-maxlength="' . (min_version(5.5) ? 2048 : 60) . '"' . ($cb ? '' : ' class="hidden"') . '>' : ''), '<p>
...
';
    ...
    if (support("partitioning")) {
        ...
        echo '<p>
', "<select name='partition_by'>" . optionlist(array("" => "") + $Je,
...
<th id="label-type">', lang(174), '<th><input type="submit" class="wayoff">', lang(175), '<th id="label-name">', lang(176), '<th><noscript>', "<input type='image' class='icon' name='add[0]' src='" . h(preg_replace("~\\?.*~",
                "", ME) . "?file=plus.gif&version=4.7.0") . "' alt='+' title='" . lang(102) . "'>", '</noscript>
</thead>
';
    if ($af) {
    ...
    foreach ($L["indexes"] as $x) {
        if (!$_POST["drop_col"] || $jd != key($_POST["drop_col"])) {
            echo "<tr><td>" . html_select("indexes[$jd][type]", array(-1 => "") + $Xc, $x["type"],
                    ($jd == count($L["indexes"]) ? "indexesAddRow.call(this);" : 1), "label-type"), "<td>";
            ksort($x["columns"]);
            $u = 1;
            foreach ($x["columns"] as $_ => $d) {
                echo "<span>" . select_input(" name='indexes[$jd][columns][$u]' title='" . lang(44) . "'",
                        ($o ? array_combine($o, $o) : $o), $d,
                        "partial(" . ($u == count($x["columns"]) ? "indexesAddColumn" : "indexesChangeColumn") . ", '" . js_escape($z == "sql" ? "" : $_GET["indexes"] . "_") . "')"), ($z == "sql" || $z == "mssql" ? "<input type='number' name='indexes[$jd][lengths][$u]' class='size' value='" . h($x["lengths"][$_]) . "' title='" . lang(100) . "'>" : ""), (support("descidx") ? checkbox("indexes[$jd][descs][$u]",
                    1, $x["descs"][$_], lang(55)) : ""), " </span>";
                $u++;
            }
            echo "<td><input name='indexes[$jd][name]' value='" . h($x["name"]) . "' autocapitalize='off' aria-labelledby='label-name'>\n", "<td><input type='image' class='icon' name='drop_col[$jd]' src='" . h(preg_replace("~\\?.*~",
                        "",
                        ME) . "?file=cross.gif&version=4.7.0") . "' alt='x' title='" . lang(105) . "'>" . script("qsl('input').onclick = partial(editingRemoveRow, 'indexes\$1[type]');");
        }
        $jd++;
    }
    echo '</table>
...
';
    ...
    page_header(DB != "" ? lang(63) : lang(109), $m, array(), h(DB));
    $Xa = collations();
    $F = DB;
    if ($_POST) {
        $F = $L["name"];
...
', ($_POST["add_x"] || strpos($F,
            "\n") ? '<textarea id="name" name="name" rows="10" cols="40">' . h($F) . '</textarea><br>' : '<input name="name" id="name" value="' . h($F) . '" data-maxlength="64" autocapitalize="off">') . "\n" . ($Xa ? html_select("collation",
                array("" => "(" . lang(95) . ")") + $Xa, $L["collation"]) . doc_link(array(
...
';
    if (DB != "") {
        ...
        echo "<input type='image' class='icon' name='add' src='" . h(preg_replace("~\\?.*~", "",
                    ME) . "?file=plus.gif&version=4.7.0") . "' alt='+' title='" . lang(102) . "'>\n";
    }
    echo '<input type="hidden" name="token" value="', $T, '">
</form>
';
...
';
    ...
    if ($_POST && !$m && !$_POST["add"] && !$_POST["change"] && !$_POST["change-js"]) {
        ...
        if ($z == "sqlite") {
            queries_redirect($C, $E, recreate_table($b, $b, array(), array(),
...
';
        ...
        foreach ($L["source"] as $_ => $X) {
            echo "<tr>", "<td>" . html_select("source[" . (+$_) . "]", array(-1 => "") + $Vf, $X,
                    ($jd == count($L["source"]) - 1 ? "foreignAddRow.call(this);" : 1),
                    "label-source"), "<td>" . html_select("target[" . (+$_) . "]", $vg, $L["target"][$_], 1,
                    "label-target");
            $jd++;
        }
        echo '</table>
<p>
', lang(97), ': ', html_select("on_delete", array(-1 => "") + explode("|", $me),
            $L["on_delete"]), ' ', lang(96), ': ', html_select("on_update", array(-1 => "") + explode("|", $me),
...
';
    ...
    if ($_POST && !$m) {
        ...
        if (!$_POST["drop"] && $b == $F && $z != "sqlite" && $U == "VIEW" && $Ae == "VIEW") {
            ...
            drop_create("DROP $Ae " . table($b), "CREATE $U " . table($F) . $ua, "DROP $U " . table($F),
                "CREATE $U " . table($xg) . $ua, "DROP $U " . table($xg), ($_POST["drop"] ? substr(ME, 0, -1) : $C),
                lang(192), $E, lang(193), $b, $F);
        }
    }
    if (!$_POST && $b != "") {
        $L = view($b);
    ...
    page_header(($b != "" ? lang(39) : lang(194)), $m, array("table" => $b), h($b));
    echo '
<form action="" method="post">
...
<tr><th>', lang(202), '<td><input type="number" name="INTERVAL_VALUE" value="', h($L["INTERVAL_VALUE"]), '" class="size"> ', html_select("INTERVAL_FIELD",
        $cd, $L["INTERVAL_FIELD"]), '<tr><th>', lang(112), '<td>', html_select("STATUS", $dg,
...
';
    ...
    if ($_POST && !process_fields($L["fields"]) && !$m) {
        ...
        drop_create("DROP $Af " . routine_id($ca, $ye), create_routine($Af, $L),
            "DROP $Af " . routine_id($L["name"], $L), create_routine($Af, array("name" => $xg) + $L),
            "DROP $Af " . routine_id($xg, $L), substr(ME, 0, -1), lang(204), lang(205), lang(206), $ca, $L["name"]);
    }
    page_header(($ca != "" ? (isset($_GET["function"]) ? lang(207) : lang(208)) . ": " . h($ca) : (isset($_GET["function"]) ? lang(209) : lang(210))),
        $m);
    if (!$_POST && $ca != "") {
...
<form action="" method="post" id="form">
<p>', lang(176), ': <input name="name" value="', h($L["name"]), '" data-maxlength="64" autocapitalize="off">
', ($Bf ? lang(19) . ": " . html_select("language", $Bf,
            $L["language"]) . "\n" : ""), '<input type="submit" value="', lang(14), '">
<div class="scrollable">
<table cellspacing="0" class="nowrap">
';
...
';
    ...
    if ($_POST) {
        if (!$m && in_array($_POST["Timing"], $Qg["Timing"]) && in_array($_POST["Event"],
            ...
            if ($_POST["drop"]) {
                ...
                if ($F != "") {
                    queries(create_trigger($le, $L + array("Type" => reset($Qg["Type"]))));
                }
            }
        }
        $L = $_POST;
    }
    page_header(($F != "" ? lang(215) . ": " . h($F) : lang(216)), $m, array("table" => $b));
    echo '
<form action="" method="post" id="form">
<table cellspacing="0" class="layout">
<tr><th>', lang(217), '<td>', html_select("Timing", $Qg["Timing"], $L["Timing"], "triggerChange(/^" . preg_quote($b,
            "/") . "_[ba][iud]$/, '" . js_escape($b) . "', this.form);"), '<tr><th>', lang(218), '<td>', html_select("Event",
...
';
        ...
        if ($_POST["drop"]) {
            ...
            if (!$m) {
                ...
                foreach ($Xd
                    ...
                    if (preg_match('~^(.+)\s*(\(.*\))?$~U', $ee, $D) && (!grant("REVOKE", $yf, $D[2],
                                " ON $D[1] FROM $Zd") || !grant("GRANT", $Dc, $D[2], " ON $D[1] TO $Zd"))) {
                        $m = true;
                        break;
                    }
                }
    ...
    page_header((isset($_GET["host"]) ? lang(31) . ": " . h("$ea@$_GET[host]") : lang(139)), $m,
...
', script("mixin(qsl('table'), {onclick: tableClick, ondblclick: partialArg(tableClick, true)});");
    ...
    foreach (process_list() as $u => $L) {
        ...
        echo "<tr" . odd() . ">" . (support("kill") ? "<td>" . checkbox("kill[]", $L[$z == "sql" ? "Id" : "pid"],
                    0) : "");
        foreach ($L
                 as $_ => $X) {
            echo "<td>" . (($z == "sql" && $_ == "Info" && preg_match("~Query|Killed~",
                        ...
                        100,
                        "</code>") . ' <a href="' . h(ME . ($L["db"] != "" ? "db=" . urlencode($L["db"]) . "&" : "") . "sql=" . urlencode($X)) . '">' . lang(227) . '</a>' : h($X));
        }
        echo "\n";
    }
    echo '</table>
...
', script("tableCheck();");
    ...
    if ($ge && !$af) {
        $af = $ah = array($ge => 0);
        $y[] = array("type" => "PRIMARY", "columns" => array($ge));
    }
    if ($_POST && !$m) {
        $vh = $Z;
        ...
        if ($_POST["export"]) {
            ...
            if (!is_array($_POST["check"]) || $af) {
                ...
                foreach ($_POST["check"] as $X) {
                    $Wg[] = "(SELECT" . limit($Bc,
                            "\nWHERE " . ($Z ? implode(" AND ", $Z) . " AND " : "") . where_check($X, $o) . $Gc,
                            1) . ")";
                }
            ...
            if ($_POST["save"] || $_POST["delete"]) {
                ...
                if ($_POST["delete"] || $P) {
                    ...
                    if ($_POST["all"] || ($af && is_array($_POST["check"])) || $gd) {
                        $J = ($_POST["delete"] ? $l->delete($b,
                            $vh) : ($_POST["clone"] ? queries("INSERT $I$vh") : $l->update($b, $P, $vh)));
                        $na = $g->affected_rows;
                    } else {
                        foreach ((array)$_POST["check"] as $X) {
                            $uh = "\nWHERE " . ($Z ? implode(" AND ", $Z) . " AND " : "") . where_check($X, $o);
                            $J = ($_POST["delete"] ? $l->delete($b, $uh,
                                1) : ($_POST["clone"] ? queries("INSERT" . limit1($b, $I, $uh)) : $l->update($b, $P,
                                $uh, 1)));
                            if (!$J) {
                                break;
                            }
                    ...
                    foreach ($_POST["val"] as $Yg => $L) {
                        ...
                        foreach ($L
                            ...
                            $P[idf_escape($_)] = (preg_match('~char|text~',
                                $o[$_]["type"]) || $X != "" ? $c->processInput($o[$_], $X) : "NULL");
                        }
                        $J = $l->update($b, $P,
                            " WHERE " . ($Z ? implode(" AND ", $Z) . " AND " : "") . where_check($Yg, $o), !$gd && !$af,
                            " ");
                        if (!$J) {
                            break;
                        }
                ...
                foreach ($Fd[0] as $_ => $X) {
                    ...
                    if (!$_ && !array_diff($Gd[1], $Ya)) {
                        ...
                        foreach ($Gd[1] as $u => $Ua) {
                            $P[idf_escape($Ya[$u])] = ($Ua == "" && $o[$Ya[$u]]["null"] ? "NULL" : q(str_replace('""',
                                '"', preg_replace('~^"|"$~', '', $Ua))));
                        }
                        $M[] = $P;
                    }
    ...
    if (isset($zf["insert"]) || !support("table")) {
        ...
        foreach ((array)$_GET["where"] as $X) {
            if ($q[$X["col"]] && count($q[$X["col"]]) == 1 && ($X["op"] == "=" || (!$X["op"] && !preg_match('~[_%]~',
                            $X["val"])))) {
        ...
        if (!$J) {
            ...
            if (!$M) {
                ...
                echo "<div class='scrollable'>", "<table id='table' cellspacing='0' class='nowrap checkable'>", script("mixin(qs('#table'), {onclick: tableClick, ondblclick: partialArg(tableClick, true), onkeydown: editingKeydown});"), "<thead><tr>" . (!$t && $N ? "" : "<td><input type='checkbox' id='all-page' class='jsonly'>" . script("qs('#all-page').onclick = partial(formCheck, /check/);",
                            "") . " <a href='" . h($_GET["modify"] ? remove_from_uri("modify") : $_SERVER["REQUEST_URI"] . "&modify=1") . "'>" . lang(235) . "</a>");
                $Wd = array();
                $Cc = array();
                reset($N);
                $mf = 1;
                    ...
                    if (!isset($ah[$_])) {
                        ...
                        if ($F != "") {
                            ...
                            echo "<th>" . script("mixin(qsl('th'), {onmouseover: partial(columnMouse), onmouseout: partial(columnMouse, ' hidden')});",
                                    ""), '<a href="' . h($Sc . ($ue[0] == $d || $ue[0] == $_ || (!$ue && $gd && $t[0] == $d) ? $Ab : '')) . '">';
                            echo
                ...
                foreach ($c->rowDescriptions($M, $q) as $Vd => $L) {
                    ...
                    foreach ($Xg
                        ...
                        if (($z == "sql" || $z == "pgsql") && preg_match('~char|text|enum|set~',
                                $o[$_]["type"]) && strlen($X) > 64) {
                            $_ = (strpos($_, '(') ? $_ : idf_escape($_));
                            $_ = "MD5(" . ($z != 'sql' || preg_match("~^utf8~",
                                    $o[$_]["collation"]) ? $_ : "CONVERT($_ USING " . charset($g) . ")") . ")";
                            $X = md5($X);
                        }
                    ...
                    echo "<tr" . odd() . ">" . (!$t && $N ? "" : "<td>" . checkbox("check[]", substr($Yg, 1),
                                in_array(substr($Yg, 1),
                                    (array)$_POST["check"])) . ($gd || information_schema(DB) ? "" : " <a href='" . h(ME . "edit=" . urlencode($b) . $Yg) . "' class='edit'>" . lang(237) . "</a>"));
                    foreach ($L
                             as $_ => $X) {
                        if (isset($Wd[$_])) {
                            $n = $o[$_];
                            ...
                            if (!$B && $X !== null) {
                                foreach ((array)$q[$_] as $p) {
                                    if (count($q[$_]) == 1 || end($p["source"]) == $_) {
                                        ...
                                        $B = ($p["db"] != "" ? preg_replace('~([?&]db=)[^&]+~',
                                                '\1' . urlencode($p["db"]),
                                                ME) : ME) . 'select=' . urlencode($p["table"]) . $B;
                                        if ($p["ns"]) {
            ...
            if (!is_ajax()) {
                ...
                if ($M || $G) {
                    if ($Fe) {
                        ...
                        echo "<fieldset>";
                        if ($z != "simpledb") {
                            echo "<legend><a href='" . h(remove_from_uri("page")) . "'>" . lang(241) . "</a></legend>", script("qsl('a').onclick = function () { pageClick(this.href, +prompt('" . lang(241) . "', '" . ($G + 1) . "')); return false; };"), pagination(0,
                                    $G) . ($G > 5 ? " ..." : "");
                            ...
                            if ($Id > 0) {
                                echo($G + 5 < $Id ? " ..." : ""), ($fc && $Ac !== false ? pagination($Id,
                                    $G) : " <a href='" . h(remove_from_uri("page") . "&page=last") . "' title='~$Id'>" . lang(242) . "</a>");
                            }
                        } else {
                            echo "<legend>" . lang(241) . "</legend>", pagination(0,
                                    $G) . ($G > 1 ? " ..." : ""), ($G ? pagination($G,
                                $G) : ""), ($Id > $G ? pagination($G + 1, $G) . ($Id > $G + 1 ? " ..." : "") : "");
                        }
                        echo "</fieldset>\n";
                    }
                    ...
                    $Fb = ($fc ? "" : "~ ") . $Ac;
                    echo
                        checkbox("all", 1, 0, ($Ac !== false ? ($fc ? "" : "~ ") . lang(143, $Ac) : ""),
...
';
                    ...
                    if ($zc) {
                        ...
                        $De = $c->dumpOutput();
                        echo($De ? html_select("output", $De, $ma["output"]) . " " : ""), html_select("format", $zc,
                            $ma["format"]), " <input type='submit' name='export' value='" . lang(69) . "'>\n", "</div></fieldset>\n";
                    }
        ...
        if ($_GET["ns"] !== "") {
            ...
            if (!$sg) {
                ...
                echo "<div class='scrollable'>\n", "<table cellspacing='0' class='nowrap checkable'>\n", script("mixin(qsl('table'), {onclick: tableClick, ondblclick: partialArg(tableClick, true)});"), '<thead><tr class="wrap">', '<td><input id="check-all" type="checkbox" class="jsonly">' . script("qs('#check-all').onclick = partial(formCheck, /^(tables|views)\[/);",
                        ...
                        'sql' => 'charset-charsets.html',
                        'mariadb' => 'supported-character-sets-and-collations/'
                    )), '<td>' . lang(253) . $Gb, '<td>' . lang(254) . $Gb, '<td>' . lang(255) . $Gb, '<td>' . lang(47) . doc_link(array(
                        'sql' => 'example-auto-increment.html',
                        'mariadb' => 'auto_increment/'
                ...
                foreach ($sg
                    ...
                    $v = h("Table-" . $F);
                    echo '<tr' . odd() . '><td>' . checkbox(($oh ? "views[]" : "tables[]"), $F, in_array($F, $tg, true),
                            "", "", "",
                ...
                echo "<tr><td><th>" . lang(228,
                        count($sg)), "<td>" . h($z == "sql" ? $g->result("SELECT @@storage_engine") : ""), "<td>" . h(db_collation(DB,
                        collations()));
                ...
                if (!information_schema(DB)) {
                    ...
                    if (count($j) != 1 && $z != "sqlite") {
                        ...
                        echo "<p>" . lang(263) . ": ", ($j ? html_select("target", $j,
                            $k) : '<input name="target" value="' . h($k) . '" autocapitalize="off">'), " <input type='submit' name='move' value='" . lang(264) . "'>", (support("copy") ? " <input type='submit' name='copy' value='" . lang(265) . "'>" : ""), "\n";
                    }
